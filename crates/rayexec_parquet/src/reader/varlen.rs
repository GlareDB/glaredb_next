                ...
                match def_levels {
                    Some(levels) => {
                        ...
                        for valid in bitmap.iter() {
                            if valid {
                                values.push(buf_iter.next().expect("buffered value to exist").take_data().expect("data to exist"))
                            } else {
                    ...
                    None => {
                        let values: Vec<_> = self.values_buffer.drain(..).map(|mut b| b.take_data().expect("data to exist")).collect();
