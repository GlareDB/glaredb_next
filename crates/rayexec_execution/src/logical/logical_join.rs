...
pub struct LogicalMagicJoin {
    ...
    pub conditions: Vec<ComparisonCondition>,
    /// Table refs generated by the magic scans on the right side of the plan.
    ///
    /// These refs remain stable across all planning/optimization steps. These
    /// exist on the join to allow optimizer rules to know what columns to look
    /// for that actually originate from the materialized plan on the left.
    pub magic_scan_refs: Vec<TableRef>,
}
