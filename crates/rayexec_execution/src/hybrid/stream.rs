...
impl<C: HttpClient> QuerySink for ClientToServerStream<C> {
...
impl<C: HttpClient> PartitionSink for ClientToServerPartitionSink<C> {
    fn push(&mut self, batch: Batch) -> BoxFuture<'_, Result<()>> {
        ...
        Box::pin(async { self.client.push(&self.query_id, 0, batch).await })
    }

    fn finalize(&mut self) -> BoxFuture<'_, Result<()>> {
        Box::pin(async { self.client.finalize(&self.query_id, 0).await })
    }
}
...
impl<C: HttpClient> QuerySource for ServerToClientStream<C> {
...
impl<C: HttpClient> PartitionSource for ServerToClientPartitionSource<C> {
    fn pull(&mut self) -> BoxFuture<'_, Result<Option<Batch>>> {
        Box::pin(async {
            // TODO
            loop {
                let status = self.client.pull(&self.query_id, 0).await?;
                match status {
